import torch
from src.diff_eq.differential_equations import ODE, SDE
...
def euler_maruyama(x0: torch.tensor, sde: SDE, num_timesteps) -> torch.tensor:
    """
    ...
    for i in range(1, num_timesteps + 1):
        t = t + h
        x = x + sde.drift(x, t) * h + root_h * sde.diffusion(t)
    return x
...
def euler_maruyama_with_steps(x0: torch.tensor, sde: SDE, num_timesteps):
    ...
    for i in range(1, num_timesteps + 1):
        t = t + h
        x = x + sde.drift(x, t) * h + root_h * sde.diffusion(x, t)
        xts[i, :, :] = x
    return x, xts
